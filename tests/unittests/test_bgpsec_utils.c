...
static void test_bgpsec_streams(void **state)
{
	...
	s_cpy->w_head = s_cpy->r_head = 0; // Reset read/write heads.
...
static void test_bgpsec_sizes(void **state)
{
	 ...
	 * last appended signature segment is skipped.
	 */
	unsigned int exp_stream_size = (5 * 6) // five secure path segments
				       +
				       (4 * 32) // four signature segments
				       +
				       12; // the size of rest (afi, nlri, ...)
	 ...
	 * matters.
	 */
	exp_stream_size = (5 * 6) // five secure path segments
		          +
		          (5 * 32) // five signature segments
		          +
		          12; // the size of rest (afi, nlri, ...)
